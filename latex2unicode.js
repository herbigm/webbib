function replaceLaTeX(s) {
    if (s.indexOf("\\") != -1) {
        for (let latex in latex2unicode) {
            latex = latex.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // escape regex
            let regex = new RegExp(latex, "g");
            s = s.replace(regex, latex2unicode[latex])
        }
        s = s.replace(/\\\w+\{(.*?)\}/gi, "$1");
    }
    s = s.replace(/\{(.*?)\}/g, "$1");
